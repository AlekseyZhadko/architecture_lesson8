...
public class BookingView implements View {
    ...
    @Override
    public void showChangeReservationTableResult(int reservationNo, int oldReservationNo) {
        if (reservationNo > 0) {
            System.out.printf("Бронирование столика успешно отменено. Номер вашей брони: #%d\n", oldReservationNo);
            System.out.printf("Столик успешно забронирован. Номер вашей брони: #%d\n", reservationNo);
        } else {
            System.out.println("Что-то пошло не так, попробуйте повторить попытку позже.");
        }
    ...
    public void changeReservationTable(int oldReservation, Date reservationDate, int tableNo, String name) {
        if (observer != null)
            observer.ChageReservationTable(oldReservation, reservationDate, tableNo, name);
    }
